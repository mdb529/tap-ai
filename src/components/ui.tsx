...
export function Card({
  ...
  return (
    <section className={`rounded-lg border border-slate-200 bg-white ${className}`}>
      {(title || right) && (
        <header className="flex items-start justify-between gap-4 border-b border-slate-100 px-4 py-3 sm:px-5 sm:py-3.5">
          <div>
...
export function Stat({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-3.5 py-3 sm:px-4 sm:py-3.5">
        ...
        <thead>
          <tr className="border-b border-slate-200">
            {head.map((h, i) => (
              <th
                key={i}
                className={`px-4 sm:px-5 ${dense ? "pb-1.5" : "pb-2"} whitespace-nowrap text-left text-[11px] font-semibold uppercase tracking-wide text-slate-500`}
              >
                {h}
              </th>
            ))}
...
export function Td({
  ...
  return (
    <td className={`px-4 py-2.5 align-top text-slate-700 sm:px-5 ${mono ? "font-mono text-xs" : ""} ${className}`}>
      {children}
    </td>
  );
}
